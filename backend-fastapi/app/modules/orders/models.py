...
class Order(Base):
    ...
    promotion_value: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(12, 2), nullable=True
    )
    total_amount: Mapped[Decimal] = mapped_column(Numeric(12, 2), nullable=False)

    zalopay_app_trans_id: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )
    zalopay_zp_trans_id: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )
...
class OrderItem(Base):
    ...
    original_unit_price: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(12, 2), nullable=True
    )
    expiry_discount_percent: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(5, 2), nullable=True
    )
    product_name: Mapped[Optional[str]] = mapped_column(String(200), nullable=True)

    order: Mapped["Order"] = relationship("Order", back_populates="items")
    batches: Mapped[List["OrderItemBatch"]] = relationship(
...
class OrderItemBatch(Base):
    ...
    order_item: Mapped["OrderItem"] = relationship(
        "OrderItem", back_populates="batches"
    )
