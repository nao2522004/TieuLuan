    ...
    <div className="card" style={{ padding: 0, overflow: "hidden" }}>
      <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--border-color)" }}>
        <h3 style={{ fontSize: "1rem", marginBottom: 12 }}>
          {isAdmin ? "📋 Toàn bộ ca làm việc (mọi chi nhánh)" : "📋 Lịch sử ca làm việc"}
        </h3>
        <div className="grid-cols-3" style={{ gap: 12 }}>
          {isAdmin && (
            <select
              ...
              value={branchFilter || ""}
              onChange={(e) => { setBranchFilter(e.target.value ? Number(e.target.value) : undefined); setPage(1); }}
            >
      ...
      {isLoading ? (
        ...
        <div style={{ textAlign: "center", padding: 40, color: "var(--text-secondary)" }}>Không có ca làm việc nào.</div>
      ) : (
        <div className="table-container">
          <table className="table">
            ...
            <tbody>
              {shifts.map((s) => (
                <tr key={s.id}>
                  <td style={{ fontWeight: 700, color: "var(--primary)" }}>#{s.id}</td>
                  <td>#{s.branch_id}</td>
                  <td>#{s.user_id}</td>
                  ...
                  <td style={{ textAlign: "right" }}>
                    <div className="flex-row-end" style={{ gap: 8 }}>
                      <button className="btn btn-secondary" style={{ padding: "4px 10px", fontSize: "0.8rem" }} onClick={() => setDetailShiftId(s.id)}>
                        Xem
                      </button>
                      {isAdmin && !s.closed_at && (
                        <button className="btn btn-danger" style={{ padding: "4px 10px", fontSize: "0.8rem" }} onClick={() => setClosingShift(s)}>
                          Đóng hộ
                        </button>
                      )}
                    </div>
      ...
      {detailShiftId && <ShiftDetailModal shiftId={detailShiftId} onClose={() => setDetailShiftId(null)} />}
      {closingShift && <CloseShiftModal shift={closingShift} onClose={() => setClosingShift(null)} />}
    </div>
  );
}